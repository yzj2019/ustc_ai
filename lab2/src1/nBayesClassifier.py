...
class NaiveBayes:
    ...
    def fit(self,traindata,trainlabel,featuretype):
        ...
        '''
        # 计算先验概率
        y_labels = np.unique(trainlabel)    # 所有标签去重
        y_counts = len(trainlabel)          # 所有数据个数
        N = len(y_labels)                   # 标签种类数
        y_label_data = {}                   # 标签对应的数据dict
        for y_label in y_labels:
            # 遍历每种分类
            self.Pc[y_label] = (trainlabel[trainlabel==y_label].shape[0] + 1) / (y_counts + N)
            y_label_data[y_label] = traindata[(trainlabel==y_label).reshape(trainlabel.shape[0],)]       # 标签为y_label的数据
        ...
        for i in range(0,traindata.shape[1]):
            ...
            x_p['data'] = {}                # 记录label->后验概率/概率分布
            for y_label in y_labels:
                # 遍历每种分类
                x_p['data'][y_label] = {}                       # 记录后验概率/概率分布
                x_i_datas = y_label_data[y_label][:,i]          # 标签为y_label的数据中，attribute i的所有数据
                if featuretype[i]:
                    # 连续型attribute，记录gauss分布的mu和sigma
                    # print(np.mean(x_i_datas))
                    x_p['data'][y_label]['mu']    = np.mean(x_i_datas)
                    # print(np.std(x_i_datas))
                    x_p['data'][y_label]['sigma'] = np.std(x_i_datas)
                else:
                    # 离散型attribute，则记录值->Laplace平滑后验概率
                    x_i_types = np.unique(traindata[:,i])
                    for x_i_type in x_i_types:
                        # 对该attribute的每个取值
                        x_p['data'][y_label][x_i_type] = (x_i_datas[x_i_datas==x_i_type].shape[0] + 1) / (y_label_data[y_label].shape[0] + len(x_i_types))
            self.Pxc[i] = x_p
    ...
    def predict(self,features,featuretype):
        ...
        需要你实现的部分
        '''       
        labels = np.empty([features.shape[0],1], dtype=int)     # 保存分类结果
        attr_num = len(featuretype)
        for i in range(0,features.shape[0]):
            # 逐条数据处理
            x = features[i]
            max_p = 0
            for y_label in self.Pc.keys():
                # 遍历每种标签
                p = 1
                for j in range(0,attr_num):
                    ...
                    if x_p['type']:
                        ...
                        mu = x_p['data'][y_label]['mu']
                        sigma = x_p['data'][y_label]['sigma']
                        p = p * np.exp(-(x_j - mu) ** 2 / (2 * sigma ** 2)) / (math.sqrt(2 * math.pi) * sigma)
                    else:
                        # 离散型
                        p = p * x_p['data'][y_label][x_j]
                p = p * self.Pc[y_label]
                if p > max_p:
                    y = y_label
                    max_p = p
